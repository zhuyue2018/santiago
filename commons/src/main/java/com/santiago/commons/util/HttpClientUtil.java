...
public class HttpClientUtil {
	...
	/**
	 * @Description:使用HttpClient发送Json数据格式的post请求
	 * @author:jjiang
	 * @time:2018年5月17日 下午3:28:23
	 */
	public static String httpClientPostJson(String urlParam, String jsonStr, String charset) {
		StringBuffer resultBuffer = null;
		HttpClient client = new DefaultHttpClient();
		HttpPost httpPost = new HttpPost(urlParam);
		// 构建请求参数
	 ...
	 * @Description:使用HttpClient发送Json数据格式的post请求
	 * @author:zhuyue
	 * @time:2019年12月17日 下午3:28:23
	 */
	public static String httpClientRealPostJson(String url, String jsonStr, Integer socketTimeOut, Integer connectTimeout) throws IOException {
		return httpClientRealPostJson(url, jsonStr, socketTimeOut, connectTimeout, 0);
	}

	public static String httpClientRealPostJson(String url, String jsonStr, Integer retryTimes) throws IOException {
		return httpClientRealPostJson(url, jsonStr, 5000, 20000, retryTimes);
	}

	public static String httpClientRealPostJson(String url, String jsonStr) throws IOException {
		return httpClientRealPostJson(url, jsonStr, 5000, 20000);
	}

	public static String httpClientRealPostJson(String url,
												String jsonStr,
												Integer socketTimeOut,
												Integer connectTimeout,
												Integer retryTimes) throws IOException {
		logger.info("url:{}", url);
		logger.info("reqStr:{}", jsonStr);
		...
		while (!postSuccess) {
			try {
				...
				if (200 == statusCode) {
					...
					throw new RuntimeException("返回状态码不是200");
				}
			} catch (Exception e) {
				logger.warn("http请求异常:{}", e.getMessage());
				if (retryCount < retryTimes) {
					retryCount ++;
					logger.warn("http请求异常重试{}次", retryCount);
				} else {
					break;
				}
			}
